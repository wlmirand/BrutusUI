...
class BrutusUIEditText(context: Context, attrs: AttributeSet) : BrutusUIGeneric<String>(context, attrs) {
    ...
    init {
        ...
        with(context.obtainStyledAttributes(attrs, R.styleable.BrutusUIGeneric)) {
            value = getText(R.styleable.BrutusUIGeneric_value)?.toString()
            recycle()
        }
    }
    ...
    private fun showDialog() {
        val dialog = with(AlertDialog.Builder(context)) {
            ...
            setPositiveButton(android.R.string.ok) { di, _ ->
                ...
                value = (di as AlertDialog).findViewById<EditText>(R.id.dialog_edittext)?.text.toString()
            }
        ...
        value?.let {
            dialog.findViewById<EditText>(R.id.dialog_edittext)?.setText(it)
        }
    }
}